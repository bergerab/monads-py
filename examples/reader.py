...
class Environment:
    ...
    def lookup(self, name):
        for sym, val in reversed(self.env):
            if name == sym.name:
                return val
        raise Exception('Name %s not found in environment' % name)

DEFAULT_ENVIRONMENT = Environment([
    [Sym('+'), lambda x: lambda y: x.val + y.val],
    [Sym('-'), lambda x: lambda y: x.val - y.val],
    [Sym('*'), lambda x: lambda y: x.val * y.val],
    [Sym('/'), lambda x: lambda y: x.val / y.val],
    [Sym('print'), lambda x: print(x.val)],
    [Sym('input'), lambda x: input(x.val)],
])
